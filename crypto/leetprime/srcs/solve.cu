...
__global__ void	brute(unsigned char *src)
{
	...
	content[2] = src[2];
	content[3] = src[3];
	content[4] = threadIdx.x;
	content[5] = blockIdx.x;
	content[6] = blockIdx.y;
	content[7] = blockIdx.z;

	SHA256_CTX ctx;
	sha256_init(&ctx);
	sha256_update(&ctx, content, SIZE);
...
int main(int argc, char** argv)
{
	...
	unsigned char *dst;
	CHECK(cudaMalloc((void **)&dst, 4));

	unsigned char prefix[4] = {0};
	sscanf(argv[1], "%02hhx%02hhx%02hhx%02hhx", &prefix[0], &prefix[1], &prefix[2], &prefix[3]);

	start = clock();
	CHECK(cudaMemcpy((void *)dst, (void *)prefix, 4, cudaMemcpyHostToDevice));
	brute<<<blocks, threads>>>(dst);
	CHECK(cudaGetLastError());
	CHECK(cudaDeviceSynchronize());

	stop = clock();
	std::cerr << "Took: " << ((double)(stop - start)) / CLOCKS_PER_SEC << "\n";	
	CHECK(cudaFree((void **)&dst));
